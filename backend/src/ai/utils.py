class NodePredictionUtils:
    ...
    number_classes = [str(i) for i in range(1, 26)]  # Numbers 1-25 as strings
    ...
    number_to_index = {num: idx for idx, num in enumerate(number_classes)}  # Mapping for numbers (1-25)
    ...
    index_to_number = {idx: num for num, idx in number_to_index.items()}  # Reverse the mapping
    ...
    @staticmethod
    def index_to_tile_value(class_type, index):
        """Get the class name from an index, returning False for specific cases."""
        class_map = {
            "letter": NodePredictionUtils.index_to_letter,
            "number": NodePredictionUtils.index_to_number,
            "powerup": NodePredictionUtils.index_to_powerup,
        ...
        if class_type in {"powerup", "double_letter", "triple_letter", "double_point"}:
            # Check the value from the corresponding mapping
            value = class_map[class_type].get(index, "NONE")  # Return None if index not found
            return value != "NONE"  # True if not "NONE", otherwise False

        # For "letter" and "number", return the mapped value directly
        return class_map[class_type].get(index, "NONE")  # Return None if index not found
